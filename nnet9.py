#!/usr/bin/env python
'''
Use 1: instead of :-1 in Unirep representation
External Switch
'''
...
class ReadSwitch():
	def __init__(self, switch, maxnum):
		self.switch = switch
		self.maxnum = maxnum

	def __call__(self):
		try:
			with open(self.switch, 'r') as sw:
				i = sw.readline().strip()
				return int(i)
		except:
			return self.maxnum
...
def train(net, loss, train_iter, test_iter, optimizer, device, num_epochs, rsw = None):
...
#	test_pkl = 'pdb25-test-500.release.contactFeatures.pkl'
	...
	rsw = ReadSwitch(NETNAME+'.conf', num_epochs)
...
#	net = torch.load('nnet-2/nnet-3500.model')
	...
	if (TEST_MODE): chk_time = time.time(); print('Walltime: %.1f seconds' % (chk_time - start_time))
	train(net, loss, train_iter, test_iter, optimizer, device, num_epochs, rsw)

	torch.save(net, NETNAME+'.final.model')
